...
void findflow()
{
  ...
  //distance
  for( i = 1 ; i <= bn ; ++ i )
    cout<<i<<" "<<w[ 1 ][ i + 1 ]<<" "<<c[ 1 ][ i + 1 ]<<"\n";
  
  for( i = 1; i <= bn ; ++ i )
    for( j = 1; j <= sn ; ++ j )
      cout<<i<<" "<<j<<" "<<w[ i + 1 ][ j + bn + 1 ]<<" "<<c[ i + 1 ][ j + bn + 1  ]<<"\n";

  for( i = 1 ; i <= sn ; ++ i )
    cout<<i<<" "<<n<<" "<<w[ i + bn + 1 ][ n ]<<" "<<c[ i + bn + 1 ][ n ]<<"\n";
  */
  return ;
}
...
void output()
{
  ...
  if( mincost == ccost ) cout<<"OPTIMAL\n";
      ...
      for(int i = 1 ; i <= bn ; ++ i )
	{
	  for(int j = 1 ; j <= sn ; ++ j )
	    {
	      cout<<f[ i + 1 ][ bn + j + 1 ] - c[ i + 1 ][ bn + j + 1 ];//<<":"<<f[ i + 1 ][ bn + j + 1 ]<<":"<<c[ i + 1 ][ bn + j + 1 ];
	      cout<<( j < sn  ? " " : "\n");
	    }
	}
    }
...
int main()
{
  ...
  //cout<<"changed flow : "<<mincost<<" "<<ccost<<"\n";
  output();
